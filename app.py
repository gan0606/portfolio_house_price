...
def main():
    ...
    # データセットの概要
        ...
        # shap分析の結果を表示
        st.write("""
                 SHAP値は、各特徴量が予測に与える影響度を表す指標です。
                 下記のSHAP Tree図は、中央線が0を表しており、左側がマイナスの影響、右側がプラスの影響を表します。
                 例えば、TotalSF(住宅の総面積)は右側に濃いピンクで大きく伸びています。
                 これは、住宅の総面積が大きくなるほど、住宅価格が上がることを意味します。
                 また、日本ではあまり一般的ではありませんが、ScreenPorch（虫よけのガラスで囲まれたテラスの面積）も右側に濃いピンクで少々伸びています。
                 これは、ScreenPorchの面積が大きくなるほど、住宅価格が上がる傾向があることを意味します。
                 一方で、BsmtUnfSF (未完成の地下室の面積) は、左側に濃いピンクで少々伸びています。
                 これは、未完成の地下室の面積が大きくなるほど、住宅価格が下がることを意味します。
                 """)
        st.write(" ")
        st.write("[注意]")
        st.write("""
                 SHAP値は、定量的な属性情報の数字的な強さを表すことができますが、定性的な属性情報の数字的な強さを表すことはできません。
                 例えば、OverallQual(家の素材と仕上げ具合)は定性的な属性情報であり、FeatureValueから住宅価格予測において重要な役割を果たしたことが確かです。
                 しかし、OverallQualのSHAP値が大きいからといって、住宅価格が上がるとは必ずしも言えません。
                 (OverallQualのSHAP値が大きいから住宅価格が下がるという可能性もあります。)
                 """)
        # スコアが良いモデルのshap値を求める
        ...
        st.write("・ScreenPorch（虫よけのガラスで囲まれたテラス）の面積が大きいほど、住宅価格が上がる傾向があることがわかりました。")
